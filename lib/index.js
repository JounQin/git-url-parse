...
function gitUrlParse(url, refs) {
    ...
    urlInfo.name = (urlInfo.pathname || urlInfo.href).replace(/(^\/)|(\/$)/g, "").replace(/\.git$/, "");
    ...
    for (const k of ["owner", "name", "ref", "filepath", "organization", "full_name"]) {
      if (urlInfo[k]) {
        urlInfo[k] = decodeURIComponent(urlInfo[k])
      }
    }
